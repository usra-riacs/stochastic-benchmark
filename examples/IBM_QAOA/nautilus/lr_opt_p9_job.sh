...
# LR_PP_opt ("Linear Ramp", no superscript) window-sticker campaign at p=9.
#
# LR_PP_opt is TQATrainer with a real evaluator, optimizing only the 2 ramp
# slope parameters -- the no-superscript "method-parameter optimization only"
# tier (as opposed to LR_PP_angle_opt, which chains in a second ScipyTrainer
# stage for full angle-by-angle refinement -- the starred tier, see
# lr_angle_opt_p5_job.sh). The existing local heavy_hex_144_LR_opt_p9_expanded
# is a stale partial run (343 rows, a single instance) rather than a full
# 30-instance campaign, so this reruns it properly.
#
# Usage:
#   ./lr_opt_p9_job.sh <setup|shards|finalize|cleanup> [--apply]
#
# Without --apply, prints the manifest to stdout (kubectl apply -f - yourself).
# With --apply, pipes directly into `kubectl apply -f -`.
#
# Sequence:
#   ./lr_opt_p9_job.sh setup --apply
#   ./lr_opt_p9_job.sh shards --apply
#   kubectl get pods -n usra-expedition -l job-name=ibm-qaoa-lr-opt-p9-shards -w
#   # once all 10 shard pods complete:
#   ./lr_opt_p9_job.sh finalize --apply

set -euo pipefail

STAGE="${1:?usage: lr_opt_p9_job.sh <setup|shards|finalize|cleanup> [--apply]}"
APPLY="${2:-}"

NAMESPACE="usra-expedition"
JOB_PREFIX="ibm-qaoa-lr-opt-p9"
OUTPUT_TAG="heavy_hex_144_LR_opt_p9_expanded"
        ...
        - name: runner
          ...
          args:
            - |
              ...
              export WORKDIR="/workspace/workdirs/lr-opt-p9-shard-\${JOB_COMPLETION_INDEX}"
        ...
        - name: runner
          ...
          args:
            - |
              ...
              export WORKDIR="/workspace/workdirs/lr-opt-p9-finalize"
              mkdir -p "\${WORKDIR}" /workspace/results /workspace/data
              rm -rf "\${WORKDIR}/repos"
              mkdir -p "\${WORKDIR}/repos"
              export REPOS_DIR="\${WORKDIR}/repos"
