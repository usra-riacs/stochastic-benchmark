...
for instance in instance_list:
    ...
    for metric in metrics_list:
        if metric + '_conf_interval' in df_results_dneal.columns:
            df_results_dneal[metric + '_conf_interval_lower'] = \
                df_results_dneal[metric +
                                 '_conf_interval'].apply(lambda x: x[0])
            df_results_dneal[metric + '_conf_interval_upper'] = \
                df_results_dneal[metric +
                                 '_conf_interval'].apply(lambda x: x[1])
...
for metric in metrics_list:
    if metric + '_conf_interval' in df_results_all.columns:
        df_results_all[metric + '_conf_interval_lower'] = \
            df_results_all[metric + '_conf_interval'].apply(lambda x: x[0])
        df_results_all[metric + '_conf_interval_upper'] = \
            df_results_all[metric + '_conf_interval'].apply(lambda x: x[1])
...
for metric in metrics_list:
    if metric + '_conf_interval' in df_results_all_stats.columns:
        df_results_all_stats[metric + '_conf_interval_lower'] = \
            df_results_all_stats[metric +
                                 '_conf_interval'].apply(lambda x: x[0])
        df_results_all_stats[metric + '_conf_interval_upper'] = \
            df_results_all_stats[metric +
                                 '_conf_interval'].apply(lambda x: x[1])
...
for instance in instance_list:
    ...
    df_results_dneal = cleanup_df(df_results_dneal)
    df_results_dneal.to_pickle(df_path)
    df_list.append(df_results_dneal)
