...
def InterpolateSingle(
    df_single: pd.DataFrame, interp_params: InterpolationParameters, group_on
):
    """
    ...
    interpolate_resource = interp_params.resource_values[
        np.where(
            (
                interp_params.resource_values
                <= take_closest(
                    interp_params.resource_values, df_single["resource"].max()
                )
            )
            & (
                interp_params.resource_values
                >= take_closest(
                    interp_params.resource_values, df_single["resource"].min()
                )
            )
        )
    ]
...
def Interpolate_reduce_mem(
    df_list: list, interp_params: InterpolationParameters, group_on
):
    """
    Complete interpolation function to include preparation, resource columns and actual interpolation.
