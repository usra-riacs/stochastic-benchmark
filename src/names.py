...
class paths:
    ...
    def __init__(self, home_dir):
        ...
        self.virtual_best = {
            "train": os.path.join(self.checkpoints, "vb_train.pkl"),
            "test": os.path.join(self.checkpoints, "vb_test.pkl"),
        }

        self.best_rec = {
            "stats": os.path.join(self.checkpoints, "br_stats.pkl"),
            "results": os.path.join(self.checkpoints, "br_results.pkl"),
        }

        self.projections = {
            "stats": os.path.join(self.checkpoints, "proj_stats.pkl"),
...
def param2filename(param_dict, ext, ignore=[]):
    ...
    def val2str(v):
        return str(v)  # update this for better float formatting

    filename = [
        "{}={}_".format(str(k), val2str(param_dict[k]))
