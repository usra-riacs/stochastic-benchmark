...
def single_experiment(
    ...
    if len(rsParams.restrict) == 0:
        ...
        df_tau = df_stats[
            (df_stats["resource"] == tau) & (df_stats[rsParams.restrict] == True)
        ].copy()
    ...
    if rsParams.optimization_dir == 1:
        exploit_df.loc[:, [rsParams.key, CIlower, CIupper]].clip(
...
def run_experiments(df_stats: pd.DataFrame, rsParams: RandomSearchParameters):
    ...
    final_values = []
    total = (
        len(rsParams.budgets)
        * len(rsParams.exploration_fracs)
        * len(rsParams.taus)
        * rsParams.Nexperiments
    )
    pbar = tqdm(
        product(
            rsParams.budgets,
            rsParams.exploration_fracs,
            rsParams.taus,
            range(rsParams.Nexperiments),
        ),
        total=total,
    )
...
def apply_allocations(
    ...
    rsParams : RandomSearchParameters
        Parameters for random search
    best_agg_alloc : pd.DataFrame
        Best allocation of exploration meta-parameters (explore frac and tau)
    ...
    for _, row in best_agg_alloc.iterrows():
        ...
        for experiment in range(rsParams.Nexperiments):
            df_experiment = single_experiment(
                df_stats, rsParams, budget, explore_frac, tau
            )
            if df_experiment is None:
                continue
            df_experiment["Experiment"] = experiment
...
def RandomExploration(df_stats: pd.DataFrame, rsParams: RandomSearchParameters):
    ...
    final_values = run_experiments(
        df_stats, rsParams
    )  # The final values (i.e., last response for each experiment)
