...
class StatsMeasure:
    ...
    def center(self, base, lower, upper):
        raise NotImplementedError("Center should be overriden by a subclass of StatsMeasure")
    def ConfIntlower(self, base, lower, upper):
        raise NotImplementedError("ConfIntlower should be overriden by a subclass of StatsMeasure")
    ...
    def ConfInts(self, base, lower, upper):
        raise NotImplementedError("ConfInts should be overriden by a subclass of StatsMeasure")
...
class Percentile(StatsMeasure):
    ...
    def ConfInts(self, base: pd.DataFrame, lower: pd.DataFrame, upper: pd.DataFrame):
        ...
        for i in range(self.nboots):
            resampler = np.random.randint(0, len(base), len(base), dtype=np.intp) # intp is indexing dtype
    ...
    for stat_measure in stat_measures:
        for key, value in lower_bounds.items():
            if str(stat_measure) + '_' + key + '_conf_interval_lower' in df_stats.columns:
                df_copy = df_stats.loc[:, (str(stat_measure) + '_' + key + '_conf_interval_lower')].copy()
                df_copy.clip(lower=value, inplace=True)
                df_stats.loc[:, (str(stat_measure) + '_' + key + '_conf_interval_lower')] = df_copy 
        for key, value in upper_bounds.items():
            if str(stat_measure) + '_' + key + '_conf_interval_upper' in df_stats.columns:
                df_copy = df_stats.loc[:, (str(stat_measure) + '_' + key + '_conf_interval_upper')].copy()
                df_copy.clip(upper=value, inplace=True)
                df_stats.loc[:, (str(stat_measure) + '_' + key + '_conf_interval_upper')] = df_copy
