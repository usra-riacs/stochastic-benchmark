...
def plot_avg_cumulatives(NQ = 44, probs=20, conf = 0.9, N_runs = 1000, method = "avg", save_fig=True):
    ...
    Arguments:
        ...
        save_fig = True (bool): Boolean to save figure
    '''
    path = "../code_for_art/normalized_prob_dist/QPSK/"+ method + "/N=" + str(N_runs) + "/"
...
def plot_hardness_full(NQ = 44, N_runs = 2000, style = "lin", save_fig = True):
    ...
    df_total_file_avg = "../code_for_art/normalized_prob_dist/QPSK/avg/all_avg/avg_pass_test_N="+str(N_runs)+".csv"
    df_total_file_opt = "../code_for_art/normalized_prob_dist/QPSK/opt/all_avg/avg_pass_test_N="+str(N_runs)+".csv"
...
#     print(len(df_total_opt["avg"]))
    ...
    if style == "log":
        ...
        ax1.plot(np.log10(N_runs_list[:-1000] + opt_find_time), df_total_opt["avg"].values[:-1000], c = "xkcd:hot pink", label = label_oh, linewidth=thickness )
        xlim = ax1.get_ylim()
        ...
        ax1.arrow( np.log10(opt_find_time* 0.127 + 37),df_total_opt["avg"].values[opt_find_time] ,np.log10(opt_find_time* 0.127 + 37+opt_find_time)-np.log10(opt_find_time* 0.127 + 37),0,  head_width=0.01, head_length=0.05, linewidth=4, color="black", length_includes_head=True)
        ax1.arrow(np.log10(opt_find_time* 0.127 + 37+opt_find_time) ,df_total_opt["avg"].values[opt_find_time] ,-(np.log10(opt_find_time* 0.127 + 37+opt_find_time)-np.log10(opt_find_time* 0.127 + 37)),0,  head_width=0.01, head_length=0.05, linewidth=4, color="black", length_includes_head=True,
                 zorder=10)
        ax1.annotate("Overhead", xy=(np.log10(280* 0.127 + 37), df_total_opt["avg"].values[opt_find_time] + 0.005 ))#, fontsize=f_size-10)
...
#     plt.show()
    ...
    path100 = "../code_for_art/normalized_prob_dist/QPSK/avg/N=" + str(100) + "/"
    path1000 = "../code_for_art/normalized_prob_dist/QPSK/avg/N=" + str(1000) + "/"
