...
def gen_log_space(
    ...
    if n > 1:  # just a check to avoid ZeroDivisionError
        ratio = ((float(upper) - float(lower)) / result[-1]) ** (
            1.0 / (n - len(result))
        )
    while len(result) < n:
...
def interpolate_df(
    ...
    if resource_values is None:
        if all_datapoints:
            ...
            resource_values = gen_log_space(
                min(df[resource_column].values),
                max(df[resource_column].values),
                default_boots // 10,
            )
        ...
        if os.path.exists(df_path_partial) and not overwrite_pickles:
            ...
            for parameter_set in set(df_values.index.to_list()):
                ...
                interpolate_resource = resource_values[
                    np.where(
                        (
                            resource_values
                            <= take_closest(
                                resource_values, maximum_boots * resource_factor
                            )
                        )
                        & (
                            resource_values
                ...
                if all_datapoints:
                    ...
                    df_interpolate = (
                        df_interpolate.interpolate(method="linear", limit_area="inside")
                        .dropna(how="all")
                        .reset_index()
                        .rename(columns={"index": resource_column})
                    )
                else:
                    df_interpolate = interp(
                        df_original.set_index(resource_column).sort_index(),
                ...
                df_interpolate["boots"] = (
                    df_interpolate[resource_column] / resource_factor
                )
                if "instance" in df.columns:
                    df_interpolate["instance"] = instance
                dataframes_instance.append(df_interpolate)
            df_interpolate = pd.concat(dataframes_instance).reset_index(drop=True)
...
def process_df_progress(
    ...
    if not use_raw_dataframes and os.path.exists(df_progress_path_end):
        ...
        df_progress_end = (
            df_progress[individual_run + compute_metrics + ["cum_reads"]]
            .loc[
                df_progress.sort_values(
                    ["cum_reads"],
                    ascending=False,
                )
                .groupby(individual_run)["cum_reads"]
                .idxmax()
            ]
            .copy()
        )
        ...
        for compute_metric in compute_metrics:
            if "inv_" in compute_metric:
                df_progress[compute_metric.replace("inv_", "")] = (
                    1 - df_progress[compute_metric] + EPSILON
                )
                expanding_metric = compute_metric
                df_progress_end[compute_metric.replace("inv_", "")] = (
                    1 - df_progress_end[compute_metric] + EPSILON
                )
            else:
                position = compute_metric.index("perf_ratio")
                df_progress[
                    compute_metric[:position] + "inv_" + compute_metric[position:]
                ] = (df_progress[compute_metric] + EPSILON)
                df_progress_end[
                    compute_metric[:position] + "inv_" + compute_metric[position:]
                ] = (df_progress_end[compute_metric] + EPSILON)
                expanding_metric = (
                    compute_metric[:position] + "inv_" + compute_metric[position:]
                )
        df_progress["best_inv_perf_ratio"] = (
            df_progress.sort_values(["cum_reads", "R_budget"])
            .expanding(min_periods=1)
            .min()[expanding_metric]
        )
        df_progress["best_perf_ratio"] = (
            1 - df_progress["best_inv_perf_ratio"] + EPSILON
        )

        if "f_explor" not in df_progress:
            df_progress["f_explor"] = df_progress["R_explor"] / df_progress["R_budget"]

        # df_progress = cleanup_df(df_progress)

        if "f_explor" not in df_progress_end:
            df_progress_end["f_explor"] = (
                df_progress_end["R_explor"] / df_progress_end["R_budget"]
            )
    ...
    df_progress_best = (
        df_progress_end.groupby(experiment_setting)[compute_metrics]
        .agg(stat_measures)
        .groupby(["R_budget"])
        .agg(opt_stats)
        .copy()
    ...
    df_progress_best.columns = [
        "_".join(reversed(pair)).replace(del_str, "")
        for pair in df_progress_best.columns
    ]
    ...
    for stat_measure in stat_measures:
        for compute_metric in compute_metrics:
            if "inv_" in compute_metric:
                df_progress_best[
                    stat_measure + "_" + compute_metric.replace("inv_", "")
                ] = (
                    1 - df_progress_best[stat_measure + "_" + compute_metric] + EPSILON
                )
            else:
                position = compute_metric.index("perf_ratio")
                df_progress_best[
    ...
    for column in df_progress_best.columns:
        if "idx" in column:
            df_progress_best = df_progress_best.merge(
                df_progress_best[column]
                .apply(pd.Series)
                .rename(
                    columns={
                        i: experiment_setting[i] for i in range(len(experiment_setting))
                    }
                ),
                on="R_budget",
                how="left",
    ...
    if "f_explor" not in df_progress_best.columns:
        df_progress_best["f_explor"] = (
            df_progress_best["R_explor"] / df_progress_best["R_budget"]
        )
